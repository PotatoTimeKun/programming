...
void draw() {
  ...
  float minAngle=PI/2;
  final float bluetaiseki=taiseki(mouseAngle, r);
  final float redtaiseki=taiseki(0, r)-bluetaiseki; // 半球-青い部分
  for (float i=PI/2; taiseki(i, r)<=redtaiseki; i-=0.001)
    minAngle=i; // 赤い部分になる角度を求める
...
float taiseki(final float angle, final float r) {
  // 半球と平面に囲まれた体積Vを求める、angle = 0 ~ pi/2
  return PI*r*r*r*(8-sin(3*angle)-9*sin(angle))/12;
}
...
void showText(float angle, float r) {
  ...
  text("体積:V = "+round(taiseki(angle, r)), 0, 100);
}
