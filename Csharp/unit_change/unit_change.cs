/*
unit_changeクラスに単位を変換するための各関数が入っています。
関数一覧
accchange:加速度
areachange:面積
lengthchange:長さ
...
*/
class unit_change{
static const double[] ACC_TANNI = {1.0, 0.27777777,0.00008466666666,0.000423333333,0.00508,0.01,0.0254,0.3048,0.44704,0.5144444,9.80665,26.8224,1609.344};
static double accchange(double from, int kigou, int to_kigou)
{
    /// <summary>
    /// 加速度の単位を変換します。
    /// 対応単位一覧
    ...
    /// </summary>
    /// <param name="from">値</param>
    /// <param name="kigou">値の単位(番号)</param>
    /// <param name="to_kigou">変換後の単位(番号)</param>
...
static const double[] AREA_TANNI = {1.0,100.0,10000.0,1000000.0,3.305785124,3.305785124,99.173554,991.736,9917.35537,0.09290304,0.00064516,0.83612736};
static double areachange(double from, int kigou, int to_kigou)
{
    /// <summary>
    /// 面積の単位を変換します。
    /// 対応単位一覧
...
static const double[] LENGTH_TANNI={1.0,149597870700.0,1852.0,0.0254,0.3048,0.9144,1604.344,3927.27,1.1818,1.1818,0.30303,0.030303,9460730472580800.0};
static double lengthchange(double from,int kigou,int to_kigou){
    /// <summary>
    /// 長さの単位を変換します。
    /// 対応単位一覧
    /// 0番="m":メートル
...
static const double[] MASS_TANNI={1.0,1000.0,1000000.0,0.000001,0.2,28.349523125,453.59237,100000.0,3.75,3750.0,37.5,600.0};
static double masschange(double from,int kigou,int to_kigou){
    /// <summary>
    /// 質量の単位を変換します。
    /// 対応単位一覧
    /// 0番="g":グラム
...
static const double[] PREFIX = {1.0,1000000000000000000000000.0,1000000000000000000000.0,1000000000000000000.0,1000000000000000.0,1000000000000.0,1000000000.0,1000000.0,1000.0,100.0,10.0,0.1,0.01,0.001,0.000001,0.000000001,0.000000000001,0.000000000000001,0.000000000000000001,0.000000000000000000001,0.000000000000000001};
static double prefixchange(double from, int kigou, int to_kigou)
{
    /// <summary>
    /// 接頭辞の単位を変換します。
    /// 対応単位一覧
...
static const double[] SPEED_TANNI = {1.0,0.00008466667,0.000423333,0.00508,0.0254,0.2777778,0.3048,0.44704,0.514444,26.8224,1.609344,299792458};
static double speedchange(double from, int kigou, int to_kigou)
{
    /// <summary>
    /// 速度の単位を変換します。
    /// 対応単位一覧
...
static const double[] TEMP_TANNI_p = {0,237.15,459.67,0.0};
static const double[] TEMP_TANNI_m = {1.0,1.0, 5.0 / 9.0, 5.0 / 9.0};
static double remperaturechange(double from, int kigou, int to_kigou)
{
    /// <summary>
    /// 温度の単位を変換します。
    /// 対応単位一覧
...
static const double[] TIME_TANNI = {1.0, 0.00000001, 0.001024, 1.0 / 50.0, 1.0 / 60.0, 60.0, 90.0, 900.0, 864.0, 60.0 * 60.0, 60.0 * 60.0 * 24.0, 60.0 * 60.0 * 24.0 * 7.0, 60.0 * 60.0* 24.0 * 10.0, 60.0 * 60.0 * 24.0 * 14.0, 2551442.27, 60.0 * 60.0 * 24.0 * 30.0, 7776000.0, 10872000.0, 31536000.0, 31556952.0, 31557600.0, 31558149.764928, 3153600000.0};
static double time_unitchange(double from, int kigou, int to_kigou)
{
    /// <summary>
    /// 時間の単位を変換します。
    /// 対応単位一覧
...
static const double[] VOLUME_TANNI = {1.0, 0.001, 0.000005, 0.000015, 0.00025, 0.000000001, 1233.48183754752, 0.00000005, 0.000016387064, 0.028316846592, 0.764554857984, 4168.181825440579584, 0.18039, 0.0018039, 0.00018039, 0.018039, 0.000018039};
static double volumechange(double from, int kigou,int to_kigou)
{
    /// <summary>
    /// 体積の単位を変換します。
    /// 対応単位一覧
